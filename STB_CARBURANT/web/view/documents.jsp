...
<body>        
    ...
    <div id="content" class="p-4 p-md-5 pt-5">
        <div class="row">
            <div class="col-sm-6">
                <h1 class="text-logo"> <i class="fa fa-list"></i> Liste des affectations conducteur-véhicule</h1>
            </div>
            <div class="col-sm-6" >

                    <a  style="float:right" href="assignments?action=add" class="add-button"  title="Ajouter une affectation "><i  class="fa fa-plus"></i></a>
        ...
        <table class="table" id="table">
            <thead>
                ...
                <th scope="col" onclick="sortTable(2)">Date d'affectation  <i class="fa fa-sort"></i></th>
                  <th scope="col" onclick="sortTable(3)">Date fin d'affectation  <i class="fa fa-sort"></i></th>
            ...
            <tbody>
                <% 
                    ...
                    %>
                    <tr  title="cliquez pour voir détail">
                        <td><%= assignments.get(i).getUser().getEmployee().getRaisonSociale() %></td>
                        <td><%= assignments.get(i).getVehicle().getCode() %></td>
                        <td><%= assignments.get(i).getAffectationDate().substring(0,16) %></td>
                        <td><%= assignments.get(i).getAffectationEndDate() %></td>
                        ...
                        <td style="text-align: center"> 
                            <a href="assignments?action=detail&id=<%= assignments.get(i).getId() %>" title="Consulter détail"  ><i style="color: green" class="fa fa-eye"></i></a> 
                            &nbsp;&nbsp;&nbsp;
                            <a href="assignments?action=update&id=<%= assignments.get(i).getId() %>" title="Modifier"  ><i style="color: blue" class="fa fa-edit"></i></a> 
                           &nbsp;&nbsp;&nbsp;
                           <a href="assignments?action=delete&id=<%= assignments.get(i).getId() %>" title="Supprimer"  onclick="return confirm('Êtes-vous sûr de vouloir supprimer?')"><i style="color: red" class="fa fa-trash"></i></a> 
                           &nbsp;&nbsp;&nbsp;
                           <a  href="assignments?action=print&id=<%= assignments.get(i).getId() %>" title="Imprimer la fiche d'affectation & l'engagement"><i style="color: #fc6d00" class="fa fa-print"></i></a>                             
        ...
        <script type="text/javascript">
            var  id='${id}';
            console.log(id);
            window.open( 'view/affectation.jsp?id='+id );
            window.open( 'view/engagement.jsp?id='+id );
            history.back();
        </script>
    </div>
    </div>
